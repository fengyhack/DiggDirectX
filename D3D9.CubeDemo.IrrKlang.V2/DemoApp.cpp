...
void App::InitObjects(void)
{
	...
	{
		{ -20.0f, 20.0f, -20.0f, GetRandomD3DColor() },
		{ -20.0f, 20.0f, 20.0f, GetRandomD3DColor() },
		{ 20.0f, 20.0f, 20.0f, GetRandomD3DColor() },
		{ 20.0f, 20.0f, -20.0f, GetRandomD3DColor() },
		{ -20.0f, -20.0f, -20.0f, GetRandomD3DColor() },
...
{
	if (IsKeyPressedAsync(VK_F12))
	{
		showFPS = !showFPS;
	}
	else if (IsKeyPressedAsync('1'))
	{
		txtPosition = DT_TOP | DT_LEFT;
	}
	else if (IsKeyPressedAsync('2'))
	{
		txtPosition = DT_TOP | DT_RIGHT;
	}
	else if (IsKeyPressedAsync('3'))
	{
		txtPosition = DT_BOTTOM | DT_LEFT;
	}
	else if (IsKeyPressedAsync('4'))
	{
		txtPosition = DT_BOTTOM | DT_RIGHT;
	}
	else if (IsKeyPressedAsync('R'))
	{
		crBackground = D3DCOLOR_XRGB(255, 0, 0);
	}
	else if (IsKeyPressedAsync('G'))
	{
		crBackground = D3DCOLOR_XRGB(0, 255, 0);
	}
	else if (IsKeyPressedAsync('B'))
	{
		crBackground = D3DCOLOR_XRGB(0, 0, 255);
	}
	else if (IsKeyPressedAsync('Y'))
	{
		crBackground = D3DCOLOR_XRGB(255, 255, 0);
	}
	else if (IsKeyPressedAsync('C'))
	{
		crBackground = D3DCOLOR_XRGB(0, 255, 255);
	}
	else if (IsKeyPressedAsync('W'))
	{
		crBackground = D3DCOLOR_XRGB(255, 255, 255);
	}
	else if (IsKeyPressedAsync('K'))
	{
		crBackground = D3DCOLOR_XRGB(0, 0, 0);
	}
	else if (IsKeyPressedAsync(VK_SPACE))
	{
	...
	else if (IsKeyPressedAsync(VK_F11))
	{
		enableSoundEfx = !enableSoundEfx;
	}
}

void App::PlayMusic(char* pszMusicFile)
{
	soundSource = soundEngine->getSoundSource(pszMusicFile);
	soundEngine->setSoundVolume(1.0f);
	soundMusic = soundEngine->play2D(soundSource, true, false, false, true);
	soundEfxCtrl = soundMusic->getSoundEffectControl();
	TrySoundEffects();
}

void App::TrySoundEffects(void)
{
	if (soundEfxCtrl != NULL)
	{
		if (enableSoundEfx)
		{
			soundEfxCtrl->enableDistortionSoundEffect(-10.0f, 5.0f);
			soundEfxCtrl->enableChorusSoundEffect(30.0f, 20.0f);
			soundEfxCtrl->enableWavesReverbSoundEffect(0.0f, 0.0f, 200.0f);
		}
		else
		{
			soundEfxCtrl->disableAllEffects();
		}
	}
}

void App::SafeRelease(void)
{
